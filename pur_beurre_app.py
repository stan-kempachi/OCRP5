...
def select_produits(category):
    ...
    category = "%" + category + "%"
    cursor.execute('USE openfoodfacts;')
    cursor.execute("""SELECT DISTINCT id, name, categories_id, nutri_score, stores, url  FROM Food WHERE name LIKE %s or categories_id LIKE %s """, (category, category))
...
def print_produit(produit):
    ...
    Nutri-score : {} \n \
    Store : {} \n \
    URL : {}".format(produit.name[1], produit.category[2], produit.nutri_score[3], produit.stores[4], produit.url[5]))
...
def recherche_substitut(produit):
    ...
    cursor.execute('USE openfoodfacts;')
    # Faire une chaîne avec les catégories, utilisées dans la requête
    recherche = (produit.category)
    # Other variables of the request
    produit_name = produit.name[1]
    produit_score = produit.nutri_score[3]
    cursor.execute("""SELECT Food.id, Food.name, Food.categories_id, nutri_score, stores, url \
     FROM Food \
     INNER JOIN Categories ON Food.categories_id = Categories.name\
     WHERE Food.name NOT LIKE %s AND Food.nutri_score < %s """,  (produit_name, produit_score))
